...
const api = {
  ...
  newDocument: () =>
    ipcRenderer.invoke('document:new') as Promise<{ title: string; content: string }>,
  autoSave: (content: string) => ipcRenderer.invoke('document:autoSave', content),
  brainstorm: (prompt: string) =>
    ipcRenderer.invoke('ai:brainstorm', prompt) as Promise<{ suggestions: string[] }>
};
